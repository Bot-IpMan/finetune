...
def main():
    ...
    load_base = input("Введіть 'y' щоб завантажити базову модель (або натисніть Enter, щоб пропустити): ").strip().lower() == 'y'

    if load_base:
        tester.load_base_model()

    results = tester.run_test_suite()
    avg_quality = tester.generate_report(results)

    print(f"\n📈 Середня оцінка якості: {avg_quality:.1f}/10")
    if avg_quality >= 6.0:
        print("✅ Модель пройшла тестування!")
    else:
