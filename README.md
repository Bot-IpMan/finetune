...
3.  Запустіть API‑сервер

    Найпростіше скористатися контейнером:

    ```sh
    docker compose up api
    ```

    За потреби сервер можна запустити й напряму:
...
`docker-compose.yml` тепер містить три сервіси:

- **finetune** – проводить навчання моделі на вашому датасеті;
- **api** – запускає FastAPI сервер, що обслуговує натреновану модель;
- **openwebui** – веб‑інтерфейс для спілкування з моделлю.

Запустіть усі сервіси командою:

```sh
docker compose up
```

Після завершення навчання API‑сервер буде доступний на
`http://localhost:8000/v1`, а Open WebUI – на `http://localhost:8080`.
У веб‑інтерфейсі потрібно створити з'єднання з URL `http://localhost:8000/v1`
та обрати модель у меню.
